...
class appliance extends EventEmitter {
  ...
  */
  ...
  getAvailableProgram(key) { return this._programCall('available.get', { programkey: key }) }

  /************
   * SETTINGS *
   ************/

  getSettings() { return this._settingCall('getList') }
   ...
   */
   ...
   registerEvents() {
     ...
     let openStream = () => {
       ...
       return new EventSource(this.api.baseUrl + bsh.expandUrl(bsh.api.events.url, { haid: this.haId }), header)
     }

     // Open the event stream
     let source = openStream()
     ...
     if (msg.type === 'CONNECTED' || msg.type === 'DISCONNECTED') {
       ...
       if (data !== undefined && data.items !== undefined) {
         ...
         for (let i in items) {
           ...
           let result = {
             type: msg.type,
             timestamp: new Date(1000 * item.timestamp),
             raw: item
           }
           if (msg.type === 'STATUS') {
             // Update local status
    ...
    source.addEventListener('STATUS', (e) => processEvent(e), false)
    source.addEventListener('NOTIFY', (e) => processEvent(e), false)
    source.addEventListener('EVENT', (e) => processEvent(e), false)
    source.addEventListener('CONNECTED', (e) => processEvent(e), false)
    source.addEventListener('DISCONNECTED', (e) => processEvent(e), false)
    source.addEventListener('KEEP-ALIVE', () => lastAlive = new Date(), false)
  }
}
